...
@section('content')
    ...
    <div class="row">
            ...
            <form action="{{ route('projects.update', ['project' => $project->id]) }}" method="POST">
                ...
                <div class="form-row">
                    <div class="form-group col-md-3">
                        ...
                        <input class="form-control" id="project_name" name="project_name" type="text" value="{{ $project->project_name }}">
                    </div>

                    <div class="form-group col-md-3">
                        <label for="employer_name">نام کارفرما:</label>
                        <input class="form-control" id="employer_name" name="employer_name" type="text" value="{{ $project->employer_name }}">
                    </div>

                    <div class="form-group col-md-3">
                        <label for="project_location">محل اجرا:</label>
                        <input class="form-control" id="project_location" name="project_location" type="text" value="{{ $project->project_location }}">
                    </div>

                    <div class="form-group col-md-3">
                        <label for="year_enforce">سال اجرا:</label>
                        <input class="form-control" id="year_enforce" name="year_enforce" type="text" value="{{ $project->year_enforce }}">
                    </div>
        ...
        <hr>
        {{-- <form action="{{ route('projects.images.edit', ['imagetest2' => $project->id]) }}" method="POST"
        enctype="multipart/form-data">
        @csrf
        <div class="col-xl-12 col-md-12 mb-4 p-4 bg-white">
        <div class="form-group col-md-12">
        <label for="image"> انتخاب لوگو</label>
        <div class="custom-file">

        <input type="file" name="image" class="custom-file-input" id="image">
        <label class="custom-file-label" for="image"> انتخاب لوگو</label>
    </div>
</div>
<button class="btn btn-outline-primary mt-5" type="submit">ویرایش</button>
</div>
                ...
                <div class="col-md-3">
                    <div class="card">
                        ...
                        <div class="card-body text-center">
                            <form action="{{ route('projects.images.destroy', ['project' => $project->id]) }}" method="post">
                                @method('DELETE')
                                @csrf
                                <input type="hidden" name="image_id" value="{{ $image->id }}">
                                <button class="btn btn-danger btn-sm mb-3" type="submit">حذف</button>
                            </form>
                            <form action="{{ route('projects.images.set_primary', ['project' => $project->id]) }}"
                                method="post">
                                @method('PUT')
                                @csrf
                                <input type="hidden" name="image_id" value="{{ $image->id }}">
